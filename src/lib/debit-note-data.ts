...
import { hasBankSnapshot, snapshotFromRecord } from "@/lib/bank-details-types";
import {
...
/** Legacy debit notes without a bank snapshot fall back to the configured default account. */
export async function enrichDebitNotePayloadBank(
  payload: DebitNotePayload,
): Promise<DebitNotePayload> {
  if (hasBankSnapshot(payload.bank)) return payload;
  const { getDefaultBankId, resolveBankSnapshot } = await import("@/actions/bank-details-actions");
  const defaultId = await getDefaultBankId();
  if (!defaultId) return payload;
  const snapshot = await resolveBankSnapshot(defaultId);
  return snapshot ? { ...payload, bank: snapshot } : payload;
}
