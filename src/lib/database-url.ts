...
function isPlaceholderDatabaseUrl(raw: string): boolean {
  return raw.includes("db.example.com") || raw.includes("user:pass@");
}

function normalizeDatabaseUrl(raw: string): string {
  if (isPlaceholderDatabaseUrl(raw)) {
    throw new Error(
      "DATABASE_URL is still a placeholder. Set a real PostgreSQL connection string in .env.local (local) or Vercel Environment Variables (production).",
    );
  }
...
export function getDatabaseUrl(): string {
  ...
  if (!url) {
    throw new Error(
      "DATABASE_URL is not set. Create a .env.local file in the project root with DATABASE_URL=postgresql://user:password@localhost:5432/your_db (local Postgres) or your Neon pooled URL.",
    );
  }
  return url;
}
