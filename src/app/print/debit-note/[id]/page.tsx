...
import { debitNoteRecordToPayload, enrichDebitNotePayloadBank } from "@/lib/debit-note-data";
...
export default async function PrintDebitNotePage({ params, searchParams }: PageProps) {
  ...
  const document = await enrichDebitNotePayloadBank(debitNoteRecordToPayload(record));
  const customerName =
