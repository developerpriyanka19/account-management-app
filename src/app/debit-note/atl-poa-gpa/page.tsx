...
export const metadata: Metadata = { title: "ATL and POA/GPA Debit Notes" };
...
export default async function AtlPoaGpaDebitNotesPage({ searchParams }: Props) {
      ...
      <div className="mx-auto w-full max-w-[1100px] space-y-4 px-4 py-6 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h1 className="text-2xl font-semibold text-[#111827]">
            {debitNoteListTitle(DebitNoteType.ATL_POA)}
          </h1>
        </header>
        <Suspense fallback={<p className="text-sm text-[#6B7280]">Loading debit notes…</p>}>
          <DebitNoteListControls
            ...
            createHref={debitNoteCreatePath(DebitNoteType.ATL_POA)}
            createLabel={debitNoteCreateLabel(DebitNoteType.ATL_POA)}
            sort={sort}
            sortDir={sortDir}
            customers={customers}
          />
          <DebitNoteListTable
            ...
            total={result.total}
            query={query}
            emptyTitle={debitNoteEmptyTitle(DebitNoteType.ATL_POA)}
          />
        </Suspense>
      </div>
    </ToastProvider>
