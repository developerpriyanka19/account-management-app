...
export default async function ServiceOrderPage({ searchParams }: Props) {
      ...
      <div className="mx-auto w-full max-w-[1100px] space-y-4 px-4 py-6 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h1 className="text-2xl font-semibold text-[#111827]">
            {debitNoteListTitle(DebitNoteType.SERVICE_ORDER)}
          </h1>
        </header>
        <Suspense fallback={<p className="text-sm text-[#6B7280]">Loading service orders…</p>}>
          <DebitNoteListControls
            ...
            createHref={debitNoteCreatePath(DebitNoteType.SERVICE_ORDER)}
            createLabel={debitNoteCreateLabel(DebitNoteType.SERVICE_ORDER)}
            sort={sort}
            sortDir={sortDir}
            customers={customers}
          />
          <DebitNoteListTable
            ...
            total={result.total}
            query={query}
            emptyTitle={debitNoteEmptyTitle(DebitNoteType.SERVICE_ORDER)}
          />
        </Suspense>
      </div>
    </ToastProvider>
